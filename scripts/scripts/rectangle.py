...
CSV_FILENAME  = "trajectory_log_rect.csv"
PNG_FILENAME  = "trajectory_result_rect.png"
...
# ============================================================
# RectangleTrajectoryGenerator
# ============================================================
class RectangleTrajectoryGenerator:
    def __init__(
        self,
...
class Live3DPlotter:
    ...
    def _run(self):
        ...
        try:
            ...
            d = np.vstack(
                [self.desired_xyz, self.desired_xyz[0]]
            )
            ...
            ax3d.set_xlim(
                *lim.get("xlim", (0.20,0.50))
            )

            ax3d.set_ylim(
                *lim.get("ylim", (-0.15,0.15))
            )

            ax3d.set_zlim(
                *lim.get("zlim", (0.15,0.35))
            )
            ...
            while not self._stop.is_set():
                ...
                # UPDATE ERROR PLOT
                # ------------------------------------------
                err_times, err_vals = self.metrics_ref.snapshot()

                if len(err_vals) > 0:

                    err_mm = np.array(err_vals) * 1000.0

                    error_line.set_data(err_times, err_mm)
...
class MetricsCalculator:
    ...
    def snapshot(self):
        """Atomically return (times, errors) copies for plotting."""
        with self._lock:
            return list(self.error_times), list(self.errors)
    ...
    def save_error_plot(self):
        ...
        plt.xlabel("Time (s)")
        plt.ylabel("Error (mm)")
        plt.title("Rectangle Tracking Error vs Time")
        ...
        plt.savefig(
            "tracking_error_vs_time_rect.png",
            dpi=150,
            bbox_inches="tight"
        )
...
# ============================================================
# Gen3RectangleTrackingNode
# ============================================================
class Gen3RectangleTrackingNode(Node):
    def __init__(self):
        super().__init__("gen3_rectangle_tracking_node")

        # Parameters
        self.declare_parameter("width",                    0.10)
        ...
        self.declare_parameter("live_sample_hz",           20.0)
        ...
        self._live_hz    = self.get_parameter("live_sample_hz").value

        self.get_logger().info(
            f"Parameters: width={self._width}, height={self._height}, "
        ...
        self._gen          = RectangleTrajectoryGenerator(
            center_x=self._center_x, center_y=self._center_y, center_z=self._center_z,
            width=self._width, height=self._height, num_points=self._num_points,
        )
        ...
        self.get_logger().info("Gen3RectangleTrackingNode initialised (waiting for late_init).")
    ...
    # ── Live tracking-error monitor ─────────────────────────
    def _desired_at_time(self, t_rel, sample_times, desired_arr):
        """Linear-interpolate the commanded EE position at elapsed time t_rel."""
        if t_rel <= sample_times[0]:
            return desired_arr[0]
        if t_rel >= sample_times[-1]:
            return desired_arr[-1]
        idx  = int(np.searchsorted(sample_times, t_rel))
        t_lo = sample_times[idx - 1]
        t_hi = sample_times[idx]
        frac = (t_rel - t_lo) / (t_hi - t_lo) if t_hi > t_lo else 0.0
        return desired_arr[idx - 1] + frac * (desired_arr[idx] - desired_arr[idx - 1])

    def _live_monitor(self, valid_desired_xyz, exec_time, stop_event, sample_hz=20.0):
        n            = len(valid_desired_xyz)
        sample_times = [exec_time * (i + 1) / n for i in range(n)]
        desired_arr  = np.array(valid_desired_xyz, dtype=float)
        ...
        while not stop_event.is_set():
            ...
            with self._lock:
                actual = list(self._current_pos)

            if len(actual) == 3:
                desired = self._desired_at_time(min(t_rel, exec_time), sample_times, desired_arr)
                err     = self._metrics.add_error(desired, actual, t_rel)
                if err is not None:
                    with self._lock:
    ...
    def run(self):
        ...
        # Phase 1: IK solve
        self.get_logger().info(
            f"Generating rectangle waypoints "
            f"({self._width*100:.1f} cm × {self._height*100:.1f} cm)…"
        )
        ...
        # Phase 3: Execute rectangle (with LIVE tracking-error monitor)
        traj = self._executor_obj.build_trajectory(valid_joint_positions)
        self.get_logger().info("Executing rectangle trajectory…")

        monitor_stop = threading.Event()
        monitor = threading.Thread(
            target=self._live_monitor,
            args=(valid_desired_xyz, self._exec_time, monitor_stop, self._live_hz),
            daemon=True,
        )

        t_start = time.time()
        monitor.start()                              # begin live error sampling
        success = self._executor_obj.execute(traj)   # blocks until done
        t_end   = time.time()

        monitor_stop.set()                           # stop live sampling
        ...
        _, live_errors = self._metrics.snapshot()
        if len(live_errors) == 0:
            self.get_logger().warn(
                "No live samples captured — falling back to post-hoc FK error."
            )
            ...
            self.get_logger().info(
                f"Captured {len(live_errors)} live tracking-error samples."
            )
...
def main(args=None):
    ...
    node = Gen3RectangleTrackingNode()
